...
## v1.2.2
- Prevent menu filter hash updates from triggering unintended page jumps by restoring the scroll position after chips are toggled.
- Keep shared filter links usable via direct hashes without allowing the browser to auto-scroll away from the grid on load.
